...
class _PRModeScreenState extends State<PRModeScreen> {
  ...
  List<double> _calcolaDischi(double pesoTotale) {
    ...
    if (pesoTotale <= pesoBilanciereVuoto || pesoTotale > 2000) return dischiDaCaricare; 
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        if (_percentualeAttuale > 0)
                          BoxShadow(color: barColor.withValues(alpha: 0.6), blurRadius: 8, spreadRadius: 1) 
                      ]
                    ),
                  ),
                ],
  ...
  Future<void> _aggiungiPercentualeCustom() async {
    ...
    String? val = await showDialog<String>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Percentuale su misura 🎯', style: TextStyle(fontWeight: FontWeight.bold)),
        content: TextField(
          controller: percController,
          keyboardType: const TextInputType.numberWithOptions(decimal: true),
          decoration: const InputDecoration(hintText: 'Es: 87.5', suffixText: '%', border: OutlineInputBorder()),
          autofocus: true,
        ),
        actions: [
    ...
    if (val != null && val.isNotEmpty) {
      ...
      if (nuovaPerc != null && nuovaPerc > 0) {
        setState(() {
          progressioneWarmup.add({'sets': '1', 'reps': '1', 'perc': nuovaPerc, 'pr': true});
          progressioneWarmup.sort((a, b) => (a['perc'] as double).compareTo(b['perc'] as double));
        });
      }
    }
  }
            ...
            children: [
              ...
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                        ...
                        children: [
                          ElevatedButton.icon(
                            style: ElevatedButton.styleFrom(
                              backgroundColor: Colors.black45, foregroundColor: Colors.blueAccent, padding: const EdgeInsets.symmetric(vertical: 16), minimumSize: const Size(double.infinity, 50),
                            ...
                            icon: const Icon(Icons.add), label: const Text('Aggiungi +2.5%', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                          ),
                          const SizedBox(height: 12),
                          ElevatedButton.icon(
                            style: ElevatedButton.styleFrom(
                              backgroundColor: Colors.black45, foregroundColor: Colors.orangeAccent, padding: const EdgeInsets.symmetric(vertical: 16), minimumSize: const Size(double.infinity, 50),
                              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12), side: const BorderSide(color: Colors.orangeAccent)),
                            ),
                            onPressed: _aggiungiPercentualeCustom, 
                            icon: const Icon(Icons.edit), label: const Text('Aggiungi % a scelta...', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                          ),
                        ]
                      ),
                    );
                  ...
                  return Card(
                    color: pesoSelezionatoPerBilanciere == pesoArrotondato ? Colors.deepOrange.withValues(alpha: 0.3) : const Color(0xFF1E1E1E),
                    child: ListTile(
