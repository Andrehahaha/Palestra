...
class _PRModeScreenState extends State<PRModeScreen> {
  ...
  Future<void> _caricaDatiMemoria() async {
    ...
    final user = FirebaseAuth.instance.currentUser;
    Map<String, double> cloudPr = <String, double>{};
    if (user != null) {
      try {
        ...
        cloudPr = _parsePrMap(doc.data()?['personal_records']);
    ...
    final localPr = prStoriciJson != null
        ? _parsePrMap(jsonDecode(prStoriciJson))
        : <String, double>{};
    iMieiPRStorici.clear();
    _mergePrMapsMax(iMieiPRStorici, cloudPr);
    _mergePrMapsMax(iMieiPRStorici, localPr);
    await prefs.setString('personal_records', jsonEncode(iMieiPRStorici));
